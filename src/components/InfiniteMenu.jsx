      ...
      this.items.map(
        item =>
          new Promise(resolve => {
            ...
            img.onerror = () => {
              // Fallback to a default image if the custom file isn't found
              const fallback = new Image();
              fallback.crossOrigin = 'anonymous';
              fallback.onload = () => resolve(fallback);
              fallback.src = 'https://images.unsplash.com/photo-1611162617474-5b21e879e113?q=80&w=1000&auto=format&fit=crop';
            };
            img.src = item.image;
          })
      )
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  const handleButtonClick = () => {
    ...
    if (activeItem.link.startsWith('http') || activeItem.link.startsWith('mailto:')) {
      window.open(activeItem.link, '_blank');
    } else if (activeItem.link.startsWith('#')) {
      if (activeItem.link === '#') return; // Prevent invalid selector error
